...
class ProgressBar:
    ...
    def update(self, current):
        ...
        sys.stdout.write(
            f"\r{self.prefix:20} [{color}{bar}{_c(RESET, '') if not use_colors() else RESET}] {percent}%"
        )
        sys.stdout.flush()
...
class HackerUI:
    ...
    def scan_footer(results_file=None):
        ...
        if use_colors():
            ...
            if results_file:
                print(
                    f"{GREEN}║{RESET}  {DIM}📄 Results:{RESET} {CYAN}{results_file}{RESET}"
                )
            print(f"{GREEN}╚{'═'*60}╝{RESET}")
        else:
            print(f"╔{'═'*60}╗")
    ...
    def matrix_effect(duration=2):
        ...
        while time.time() < end_time:
            line = "".join(
                random.choice(chars) if random.random() > 0.5 else " "
