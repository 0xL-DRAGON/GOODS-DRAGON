...
class CVEScanner:
                ...
                for cpe in cpes[:5]:
                    ...
                    if cve_resp.status_code == 200:
                        ...
                        for vuln in cve_data.get('vulnerabilities', []):
                            ...
                            description = vuln.get('cve', {}).get('descriptions', [{}])[0].get('value', 'N/A')
                            ...
                            if cve_id != 'N/A':
                                ...
                                log_success(f"🔥 Found CVE: {cve_id} for {tech_name} (Severity: {severity})")
            else:
                if self.verbose:
                    log_debug(f"NVD API returned status {resp.status_code} for {tech_name}")
    ...
    def check_techs(self):
        ...
        if self.cms_data and isinstance(self.cms_data, dict):
            cms_name = self.cms_data.get('detected', ['Unknown'])[0] if self.cms_data.get('detected') else None
            if cms_name:
