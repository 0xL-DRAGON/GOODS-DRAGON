...
class RaceConditionDetector:
    ...
    def test_race_condition(self, url, method="GET", data=None, count=20):
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            futures = [
                executor.submit(self.send_request, url, method, data)
        ...
        if len(set(statuses)) > 1:
            ...
            with self.lock:
                ...
                log_success(
                    f"🔥 Race condition possible: {url} (statuses: {set(statuses)})"
                )
        elif self.verbose:
        ...
        try:
            ...
            if "form" in resp.text.lower():
                self.test_race_condition(
                    self.target, "POST", data={"test": "1"}, count=20
                )
        except:
            pass

        log_success(
            f"Race Condition Detection completed. Found {len(self.results)} issues."
        )
        return {
            "target": self.target,
