...
class ChainedAttackScanner:
    ...
    def check_xss_to_sqli(self):
        ...
        try:
            ...
            for payload in payloads:
                ...
                if resp.status_code == 200 and ("sql" in resp.text.lower() or "error" in resp.text.lower()):
                    self.results.append({
                        "chain": "XSS -> SQLi",
                        "payload": payload,
                        "url": test_url,
    ...
    def check_sqli_to_rce(self):
        ...
        try:
            ...
            for payload in payloads:
                ...
                resp = requests.get(test_url, timeout=5, allow_redirects=False)
                if resp.status_code == 200 and ("shell" in resp.text.lower() or "system" in resp.text.lower()):
                    self.results.append({
                        "chain": "SQLi -> RCE",
                        "payload": payload[:50],
                        "url": test_url,
    ...
    def check_lfi_to_rce(self):
        ...
        try:
            ...
            for payload in payloads:
                ...
                resp = requests.get(test_url, timeout=5, allow_redirects=False)
                if resp.status_code == 200 and ("root:" in resp.text or "base64" in resp.text):
                    self.results.append({
                        "chain": "LFI -> RCE",
                        "payload": payload,
                        "url": test_url,
    ...
    def run(self):
        ...
        log_success(f"Chained Attack Scanner completed. Found {len(self.results)} chains.")
        return {
            "target": self.target,
