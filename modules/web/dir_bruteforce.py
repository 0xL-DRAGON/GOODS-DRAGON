...
class DirBruteforce:
    ...
    def __init__(self, target: str, wordlist_path: str = None, threads: int = 30, verbose: bool = False):
        ...
        # ---------- EXTENSIONS TO CHECK ----------
        self.extensions = ['', '.php', '.html', '.htm', '.asp', '.aspx', '.jsp', '.do', '.action', '.cgi', '.pl', '.py', '.rb', '.sh', '.txt', '.xml', '.json', '.yml', '.yaml', '.ini', '.conf', '.config', '.bak', '.old', '.backup', '.swp', '.tmp', '.log']
    ...
    def _load_internal_wordlist(self) -> List[str]:
        ...
        common_dirs = [
            ...
            "api", "v1", "v2", "v3", "test", "dev", "stage", "old", "new",
            "assets", "static", "media", "public", "private", "secure",
            "hidden", "secret", "internal", "staff", "employee", "adminpanel",
            "admincp", "adminarea", "administrator", "webadmin", "backend",
            "cms", "portal", "dashboard", "controlpanel", "control",
            "system", "sysadmin", "root", "superuser", "manager",
            "support", "help", "faq", "about", "contact", "team",
            "careers", "jobs", "news", "blog", "forum", "community",
            "chat", "live", "stream", "video", "audio", "music",
            "gallery", "photos", "images", "pics", "files", "docs",
            "documents", "downloads", "uploads", "backups", "archive",
            "temp", "tmp", "cache", "sessions", "cookies", "log",
            "logs", "debug", "error", "access", "audit", "history",
            "dumps", "sql", "mysql", "postgres", "mongo", "redis",
            "elastic", "search", "index", "sitemap", "robots",
            "well-known", ".well-known", "phpmyadmin", "pma", "mysqladmin",
            "phpinfo", "info", "status", "health", "ping", "alive",
            "test", "testing", "demo", "sample", "example", "dev",
            "staging", "stage", "prod", "production", "live", "current",
            "old", "new", "latest", "version", "releases", "build",
            "deploy", "ci", "cd", "pipeline", "jenkins", "github",
            "gitlab", "bitbucket", "svn", "git", "hg", "svn",
            "docs", "doc", "manual", "guide", "tutorial", "help",
            "support", "feedback", "bug", "issue", "ticket",
            "security", "privacy", "terms", "legal", "policy",
            "copyright", "license", "credits", "authors", "contributors",
            "api", "rest", "soap", "graphql", "odata", "json",
            "xml", "rpc", "ws", "webservice", "service", "services",
            "auth", "login", "signin", "signup", "register", "logout",
            "password", "reset", "forgot", "recover", "unlock",
            "profile", "account", "settings", "preferences", "config",
            "setup", "install", "update", "upgrade", "migrate",
            "seed", "populate", "init", "bootstrap", "start",
            "stop", "restart", "reload", "flush", "clear",
            "dashboard", "home", "index", "default", "main", "master",
            "frontend", "backend", "client", "server", "web",
            "mobile", "android", "ios", "iphone", "ipad",
            "desktop", "windows", "mac", "linux", "unix",
            "admin", "moderator", "editor", "author", "contributor",
            "subscriber", "member", "guest", "visitor", "user",
            "customer", "client", "partner", "vendor", "supplier",
            "distributor", "reseller", "affiliate", "referral", "ambassador",
        ]
        ...
        sensitive_files = [
            ...
            ".htaccess", ".htpasswd", ".htgroups", ".htusers",
            ".ssh/id_rsa", ".ssh/id_dsa", ".ssh/authorized_keys",
            ".aws/credentials", ".aws/config", ".aws/credentials",
            "config.php", "wp-config.php", "settings.py", "settings.ini",
            ...
            "Cargo.toml", "go.mod", "go.sum", "pubspec.yaml",
            "Dockerfile", "docker-compose.yml", "docker-compose.yaml",
            "Jenkinsfile", ".travis.yml", ".gitlab-ci.yml", ".github/workflows/main.yml",
            ".circleci/config.yml", ".drone.yml", "build.gradle", "pom.xml",
            "web.config", "app.config", "application.properties", "application.yml",
            "log4j.properties", "logback.xml", "logging.properties",
            "server.xml", "context.xml", "web.xml", "jboss-web.xml",
            "php.ini", "php.ini-production", "php.ini-development",
        ...
        # ----- ADMIN PANELS -----
        admin_panels = [
            "admin", "administrator", "admincp", "adminpanel",
            "wp-admin", "joomla-admin", "drupal-admin", "magento-admin",
            "cpanel", "whm", "webmail", "plesk", "directadmin",
            "phpmyadmin", "pma", "mysql", "myadmin", "adminer",
            "phpinfo", "info", "phpinfo.php", "info.php", "test.php",
            "install", "setup", "config", "configure", "install.php",
            "upgrade", "update", "migrate", "deploy", "build",
        ]
        ...
        backup_files = [
            ...
            "backup.bz2", "backup.7z", "backup.rar", "backup.tgz",
            "dump.sql", "dump.zip", "dump.tar", "dump.gz",
            "db.sql", "db.zip", "db.tar", "db.gz",
            "database.sql", "database.zip", "database.tar", "database.gz",
            "data.sql", "data.zip", "data.tar", "data.gz",
            "old.zip", "old.tar", "old.gz", "old.sql",
            "new.zip", "new.tar", "new.gz", "new.sql",
            "backup_old.zip", "backup_old.tar", "backup_old.gz", "backup_old.sql",
            "backup_new.zip", "backup_new.tar", "backup_new.gz", "backup_new.sql",
            "backup_2024.zip", "backup_2024.tar", "backup_2024.gz", "backup_2024.sql",
            "backup_2025.zip", "backup_2025.tar", "backup_2025.gz", "backup_2025.sql",
            "site_backup.zip", "site_backup.tar", "site_backup.gz", "site_backup.sql",
            "db_backup.zip", "db_backup.tar", "db_backup.gz", "db_backup.sql",
            "full_backup.zip", "full_backup.tar", "full_backup.gz", "full_backup.sql",
            "daily_backup.zip", "daily_backup.tar", "daily_backup.gz", "daily_backup.sql",
            "weekly_backup.zip", "weekly_backup.tar", "weekly_backup.gz", "weekly_backup.sql",
        ]
        ...
        api_endpoints = [
            ...
            "soap", "api/soap", "wsdl", "api/wsdl",
            "json", "api/json", "xml", "api/xml",
            "rpc", "api/rpc", "odata", "api/odata",
        ]
    ...
    def run(self) -> Dict:
        ...
        log_info(f"Testing {len(self.wordlist)} paths with {len(self.extensions)} extensions")
