            ...
            for waf, signatures in self.waf_signatures.items():
                ...
                for header in signatures["headers"]:
                    if header.lower() in headers:
                        self.detected.append({"waf": waf, "type": "header", "signature": header})
                        detected = True
                ...
                for cookie in signatures["cookies"]:
                    if cookie.lower() in cookies:
                        self.detected.append({"waf": waf, "type": "cookie", "signature": cookie})
                        detected = True
                ...
                for pattern in signatures["response"]:
                    if pattern.lower() in html:
                        self.detected.append({"waf": waf, "type": "response", "signature": pattern})
                        detected = True
                        log_success(f"🔥 Detected WAF: {waf} (response pattern: {pattern})")
