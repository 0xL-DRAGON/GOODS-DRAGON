...
class ProxyServer:
    ...
    def setup_routes(self):
        ...
        def proxy(path):
            ...
            target_url = request.headers.get("X-Target-URL") or request.args.get(
                "target"
            )
            if not target_url:
            ...
            params = (
                ...
                else {k: v for k, v in request.args.items() if k != "target"}
            )
    ...
    def run(self):
        ...
        log_info(
            f"Use header X-Target-URL: https://example.com or ?target=https://example.com"
        )
        self.app.run(host="0.0.0.0", port=self.port, threaded=True)

    def get_logs(self):
        return {
            "requests": self.requests_log[-20:],
            "responses": self.responses_log[-20:],
        }
