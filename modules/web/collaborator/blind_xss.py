...
class BlindXSSCollaborator:
    ...
    def test_blind_xss(self):
        ...
        log_info(
            f"Testing {len(params)} parameters with {len(self.payloads)} blind XSS payloads..."
        )
    ...
    def run(self):
        ...
        log_info(
            "Use: https://collaborator.example.com/ or https://xsshunter.com/ to check for hits"
        )

        self.test_blind_xss()

        log_success(
            f"Blind XSS scan completed. Found {len(self.results)} potential vulnerabilities."
        )
        return {
            "target": self.target,
            "scan_type": "blind_xss",
