...
class AutoScanner:
    def __init__(
        self, target, verbose=False, threads=30, output="reports/auto_scan.json"
    ):
        self.target = target
    ...
    def run_web(self):
        ...
        dir_bf = DirBruteforce(
            target_url, "wordlists/dirs.txt", self.threads, self.verbose
        )
    ...
    def run(self):
        ...
        try:
            ...
            log_success(
                f"Duration: {int((datetime.now() - self.start_time).total_seconds())} seconds"
            )
