...
class BrokenLinkChecker:
    ...
    def check_link(self, url):
        ...
        try:
            ...
            if resp.status_code >= 400:
                result = {"url": url, "status": resp.status_code, "type": "broken"}
            ...
            result = {"url": url, "status": "ConnectionError", "type": "broken"}
            self.broken_links.append(result)
            log_warning(f"💔 Connection error: {url}")
        except requests.exceptions.Timeout:
            result = {"url": url, "status": "Timeout", "type": "broken"}
            self.broken_links.append(result)
            log_warning(f"💔 Timeout: {url}")
        except Exception as e:
            result = {"url": url, "status": str(e), "type": "broken"}
            self.broken_links.append(result)
    ...
    def run(self):
        ...
        log_success(
            f"Broken link check completed. Found {len(self.broken_links)} broken links."
        )
        return {
            "target": self.target,
            "scan_type": "broken_link",
