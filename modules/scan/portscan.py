...
class PortScanner:
            ...
            if result == 0:
                ...
                with self.lock:
                    self.open_ports.append(
                        {"port": port, "service": service, "banner": banner_text}
                    )
    ...
    def grab_banner_info(self, port):
        try:
            ...
            if port == 80 or port == 8080:
                ...
                pass
            banner = (
                sock.recv(256).decode("utf-8", errors="ignore").strip().split("\n")[0]
            )
            sock.close()
            return banner[:100]
        except:
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            futures = {
                executor.submit(self.scan_port, port): port for port in self.port_list
            }
