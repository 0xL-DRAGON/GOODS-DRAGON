    ...
    def validate_phone(self):
        ...
        if self.results.get("country_code") == "98":
            if len(self.phone) == 10:
                ...
                self.results["validation_msg"] = (
                    "Valid Iranian phone number (10 digits)"
                )
            elif len(self.phone) == 11 and self.phone.startswith("98"):
                self.results["valid"] = True
                self.results["validation_msg"] = (
                    "Valid Iranian phone number (11 digits, including country code)"
                )
            else:
                self.results["valid"] = False
                self.results["validation_msg"] = (
                    "Invalid Iranian phone number (must be 10 or 11 digits)"
                )
        else:
