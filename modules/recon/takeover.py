...
class SubdomainTakeover:
        ...
        try:
            ...
            for service, sig in self.signatures.items():
                if service in cname.lower():
                    ...
                    try:
                        resp = requests.get(f"http://{subdomain}", timeout=5, allow_redirects=False)
    ...
    def run(self):
        ...
        log_info(f"Checking {len(self.subdomains)} subdomains for takeover vulnerabilities...")
        for sub in self.subdomains:
            self.check_takeover(sub)

        if self.vulnerable:
            log_success(f"Found {len(self.vulnerable)} subdomains vulnerable to takeover.")
        else:
