...
from core.logger import (log_debug, log_error, log_info, log_success,
                         log_warning)
from core.network import check_http_status, resolve_domain
...
class SubdomainFinder:
    ...
    def check_subdomain(self, sub):
        ...
        with self.lock:
            ...
            if self.total_tested % 20 == 0:
                log_info(
                    f"Tested: {self.total_tested} - Found: {len(self.found_subdomains)}"
                )
    ...
    def run(self):
        ...
        log_success(
            f"Scan completed. Total: {len(self.found_subdomains)} - Alive: {len(alive)}"
        )
        return {
            "target": self.domain,
