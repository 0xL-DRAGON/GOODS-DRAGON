        ...
        for attempt in range(self.retries):
            try:
                ...
                if self._should_retry(resp.status_code):
                    log_warning(
                        f"Blocked ({resp.status_code}), retrying... ({attempt+1}/{self.retries})"
                    )
                ...
                log_warning(
                    f"Connection error, retrying... ({attempt+1}/{self.retries})"
                )
                self._random_delay(2.0, 6.0)
            except Exception as e:
                log_error(
                    f"Request failed: {e}, retrying... ({attempt+1}/{self.retries})"
                )
    ...
    def get_async_bulk(self, urls, params_list=None):
        ...
        for i, url in enumerate(urls):
            if params_list[i]:
                ...
                full_urls.append(
                    urllib.parse.urlunparse(parsed._replace(query=new_query))
                )
            else:
