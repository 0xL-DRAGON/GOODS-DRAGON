...
class BrowserEmulator:
    def __init__(self, target, verbose=False, headless=True, use_selenium=True, proxy_list=None):
        self.target = target
        ...
        for attempt in range(retries):
            try:
                ...
                if method.upper() == "GET":
                    ...
                    resp = self.session.post(url, data=data, proxies=proxy_dict, timeout=15)
                
                if resp.status_code == 200:
                    log_success(f"Fetched {url} with requests (Status: {resp.status_code})")
                    return resp
