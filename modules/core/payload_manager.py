...
class PayloadManager:
            ...
            for payload in payloads:
                if isinstance(payload, dict):
                    if "id" not in payload:
                        payload["id"] = hashlib.md5(
                            payload.get("value", "").encode()
                        ).hexdigest()[:8]
                    if "hash" not in payload:
                        payload["hash"] = hashlib.md5(
                            payload.get("value", "").encode()
                        ).hexdigest()
        ...
        return {
            "xss": [
                ...
                },
                {
                    ...
                    "type": "body",
                    "tags": ["alert", "body", "onload"],
                    "severity": "medium",
                    "id": "xss_007",
                },
                {
                    "value": "<input/onfocus=alert(1)>",
                    "type": "input",
                    "tags": ["alert", "input", "onfocus"],
                    "severity": "medium",
                    "id": "xss_008",
                },
                {
                ...
                {
                    "value": "<math><maction actiontype=statusline# xss=alert(1)>",
                    "type": "math",
                    "tags": ["alert", "math"],
                    "severity": "low",
                    "id": "xss_010",
        ...
        if os.path.exists(self.LOCAL_DB_PATH):
            try:
                with open(self.LOCAL_DB_PATH, "r", encoding="utf-8") as f:
                    ...
                    log_success(
                        f"Loaded payloads from local database: {self.metadata.get('version', 'unknown')}"
                    )
                    self._calculate_hashes()
    ...
    def _save_local(self):
        ...
        self._count_payloads()
        self.metadata["last_updated"] = datetime.now().isoformat()
        data = {"metadata": self.metadata, "payloads": self.payloads}
        try:
    ...
    def update_from_remote(self, url: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self._save_local()
                log_success(
                    f"Update successful! Total payloads: {self.metadata.get('total_payloads', 0)}"
                )
                return True
            else:
    ...
    def get_payloads(
        ...
        if tags:
            results = [
                p for p in results if any(tag in p.get("tags", []) for tag in tags)
            ]
    ...
    def add_payload(self, category: str, payload: Dict) -> bool:
        ...
        if "id" not in payload:
            payload["id"] = hashlib.md5(payload.get("value", "").encode()).hexdigest()[
                :8
            ]
    ...
    def remove_payload(self, category: str, payload_id: str) -> bool:
        ...
        self.payloads[category] = [
            p for p in self.payloads[category] if p.get("id") != payload_id
        ]
    ...
    def export_payloads(
        self, format_type: str = "json", output_file: str = None
    ) -> Optional[str]:
        """Export payloads in various formats"""
        if not output_file:
        ...
        if format_type == "json":
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                for category, payloads in self.payloads.items():
                    ...
                    for payload in payloads:
                        f.write(
                            f"[{payload.get('id', '')}] {payload.get('value', '')}\n"
                        )
            log_success(f"Exported payloads to: {output_file}")
            return output_file
